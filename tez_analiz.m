...
for i=3:n
    ...
    er1(i)=errorbar(zaman,Dmg(i,:),Yerr(i,:),'o',"color",mark(i,:));hold on
    plot(x,yexp(i,:),"color",mark(i,:),'LineWidth',1.5)
    ylabel('Toplam Geçirgenlik Kaybı (%)'),%title('3.5 kGy Işınlanmış Örnekler')
    xlabel('Işınlama Sonrası Geçen Süre (gün)')
    ...
    grid on
    if i==7
%         legend([er1(3:7)],name_legend{3:7})
...
%         Lm(i,1)=mean([Lx(i,1)*Ly(i,1),Lx(i,2)*Ly(i,2)])/(Ly(i,1)+Ly(i,2));
        Lm(i,1)=(Lx(i,1)*Ly(i,1)+Lx(i,2)*Ly(i,2))/(Ly(i,1)+Ly(i,2));
    end
end
...
for i=1:n
    ...
    er1(i)=errorbar(zaman,Dmgfix(i,:),Yerr(i,:),'o',"color",mark(i,:));hold on
